...
class P_rodenburg(ProxOperator):
    ...
    def work(self, u):
        ...
        ----------
        u : Pty
            Ptychography has special needs, so u must be a Pty.
        ...
        for pos in numpy.random.permutation(self.N_pie):
            ...
            if switch_probemask == True:
                probe_norm = scipy.linalg.norm(this_probe,'fro') / \
                                math.sqrt(numpy.size(this_probe));
...
class PtychographyStats():
    ...
    def _dftups(self, inp, nor=None, noc=None, usfac=1, roff=0, coff=0):
        ...
        nor, noc : int, optional
            Number of pixels in the output unsampled DFT (Default = inp.shape)
        usfac : number, optional
            Upsampling factor (Default = 1)
        roff, coff : int, optional
            Row and column offsets (Default = 0)
        """
    ...
    def _dftregistration(self, buf1ft, buf2ft, usfac=1):
        ...
        ----------
        buf1ft : array_like
            Fourier transform of the reference image
        buf2ft : array_like
            Fourier transform of the image to register
        usfac : int, optional
            Upsampling factor
        ...
        number
            Pixel shifts between images (rows)
        number
            Pixel shifts between images (columns)
        ...
        if usfac == 0:
            ...
            diffphase = math.atan2(ccmax.imag,ccmax.real);
            ...
            if usfac > 2:
                ...
                dftshift = numpy.fix(math.ceil(usfac*1.5)/2);
                cc = numpy.conj(self._dftups(buf2ft*numpy.conj( \
                        buf1ft),math.ceil(usfac*1.5),math.ceil(usfac*1.5), \
...
class Blocking_meta(Algorithm):
    ...
    def _do_view(self, u, view, tol):
        ...
        sequence of array_like
            A tuple of the form (phi,obj,probe)
        """
...
class RAAR_PALM(Algorithm):
    ...
    def run(self, u, tol, maxiter):
        ...
        num_custom_errors = self.custom_error.customerror(u,u).shape[0];
        custom_errors = numpy.zeros((num_custom_errors,maxiter+1));
        custom_errors[:,0] = 999;
...
class Ptychography(Problem):
    ...
    def __init__(self,config=default_config):
        ...
        config : dict, optional
            ...
            Ptychography.default_config is used.
        """
    ...
    def _gaussu(self, rho, z, l, wnull):
        """
        Berechnet die el. Feldkomponente eines Gaußschen Strahles
        ...
        w = wnull * math.sqrt(1 + (z**2)/(znull**2));
    ...
    def _prop_nf_pa(self,im,l,delta_z,dx,dy):
        ...
        alpha = max([math.atan(Nx/dx*(2*delta_z)),math.atan(Ny/dy*(2*delta_z))]);
        if alpha > alpha_th:
        ...
        if f > 1:
            ...
            Nx_l = math.ceil(beta*f)*Nx;
            dqx = 2*numpy.pi/(Nx_l*dx);
            Ny_l = math.ceil(beta*f)*Ny;
            ...
            im_l[Cy+math.ceil(Ny/2.0)-1:Ny_l,0:Cx-(Nx//2)-1] = im[-1,0];
            im_l[Cy+math.ceil(Ny/2.0)-1:Ny_l,Cx+math.ceil(Nx/2.0)-1:] = im[-1,-1];
            
            im_l[Cy-(Ny//2)-1:Cy+math.ceil(Ny/2.0)-1,Cy-(Ny//2)-1:Cy+math.ceil(Ny/2.0)-1] = im;
            
            Im_l = scipy.fftpack.fftshift(scipy.fftpack.ifft2( \
                    scipy.fftpack.fft2(im_l) * numpy.exp(1j*kappa*delta_z)));
            return Im_l[Cy-(Ny//2)-1:Cy+math.ceil(Ny/2.0)-1,Cy-(Ny//2)-1:Cy+math.ceil(Ny/2.0)-1];
        else:
    ...
    def _get_ptychography_data(self):
        ...
        if self.config['sim_data_type'] == 'gaenseliesel':
            g = pyplot.imread('gaenseliesel.png').astype(numpy.float);
            g /= numpy.amax(g);
            g = 0.8*g + 0.1;
...
class Ptychography_NTT_01_26210(Ptychography):
    ...
    def _get_ptychography_data(self):
        data = scipy.io.loadmat('data_NTT_01_26210_192x192.mat');
        d1y = data['d1y'][0][0];
        d1x = data['d1x'][0][0];
        i_exp = data['I_exp'];
        l = data['lambda'][0][0];
        ...
        data = scipy.io.loadmat('reconstruction_data_NTT_01_26210_192x192.mat');
        probe = data['Probe'];
        sample_plane = data['object'];
        
        data = scipy.io.loadmat('positions_NTT_01_26210.mat')
